...
> **Status: already executed (do not re-run Task 3's Singleton YAML verbatim).** This plan has been fully implemented, reviewed, and merged. Post-merge PR review found real bugs in the three Singleton rules as Task 3 originally specified them: `singleton-quality-rust` matched at whole-file scope with no type correlation between the `static INSTANCE` and the `impl` block (a false-positive risk fixed, then itself over-narrowed and re-fixed); `singleton-quality-typescript` detected "the constructor" via a literal pattern template that silently failed to match any parameterized constructor; and both Java and TypeScript missed the "no explicit constructor at all" case. **`src/default_patterns.rs` is the authoritative, corrected source for all three** -- see the addendum in `.scratch/norma-architecture/issues/06-gof-pattern-set-v2-scope.md` for the full history and the corrected YAML. If this plan is ever consulted again (e.g. as a template for a similar future change), copy the Singleton rules from the shipped source, not from Task 3 below.
