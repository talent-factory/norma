...
- [x] Automatic release tagging + GitHub Releases with prebuilt binaries -- on `main` only (never `develop`, which stays "Unreleased"), `changelog.yml`'s "Tag today's release" step tags the merge with a date-based release tag (`vYYYY.MM.DD`, disambiguated with a `.2`/`.3`/... suffix for more than one release the same day) *before* running git-cliff, so shipped code renders under a real dated section instead of a permanent "Unreleased" -- without a tag, git-cliff has no boundary to split on. It then explicitly dispatches `.github/workflows/release.yml` via `gh workflow run` (a plain tag push authenticated with `GITHUB_TOKEN` does not trigger other workflows -- GitHub's deliberate anti-recursion safeguard; `workflow_dispatch` is the documented exception). `release.yml` builds native binaries for Linux (`x86_64-unknown-linux-gnu`) and Windows (`x86_64-pc-windows-msvc`), plus a macOS *universal* binary (both `x86_64-apple-darwin` and `aarch64-apple-darwin`, combined with `lipo`), packages each with the README and both LICENSE files, and publishes them to a GitHub Release for that tag with git-cliff-generated release notes (`--latest --strip all`). Since the workflow now commits `CHANGELOG.md` independently on both `main` and `develop`, a later `develop` -> `main` PR can hit a genuine textual merge conflict in it (both sides changed the same lines differently) -- `.gitattributes` marks it `merge=union` so this doesn't block a merge; if it ever does anyway (e.g. GitHub's server-side PR merge doesn't honor the attribute), the fix is `git-cliff --config cliff.toml --output CHANGELOG.md` after merging, not hand-resolving the conflict markers -- the file is fully machine-generated either way.
