...
- [x] Automatic `CHANGELOG.md` generation -- [git-cliff](https://github.com/orhun/git-cliff) (`cliff.toml`) builds it from the full commit history, grouped by commit type (`feat`/`fix`/`docs`/`wayfinder`/...), with `(TF-xxx)` references linked to Linear; not restricted to strict Conventional Commits since this repo's early history predates that convention, so everything else falls into a catch-all group rather than being silently dropped. `.github/workflows/changelog.yml` regenerates and commits it on every push to `main` (i.e. every `develop` -> `main` merge), guarded against re-triggering itself via `paths-ignore: CHANGELOG.md` on the workflow trigger rather than an in-job check.
